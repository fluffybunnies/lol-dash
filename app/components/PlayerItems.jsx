...
export default class PlayerItems extends React.Component {
	render() {
		return <div className="PlayerItems">
			{this.props.itemIds.map((itemId, itemIndex) => {
				return <img className="PlayerItems-item" src={itemIcon(itemId)} alt={itemName(itemId)} title={itemName(itemId)} key={itemIndex} />
			})}
		</div>
	}
}
